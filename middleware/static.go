...
// root directory.
func Static(root string) pisces.MiddlewareFunc {
...
// See `Static()`.
func StaticWithConfig(config StaticConfig) pisces.MiddlewareFunc {
	// Defaults
	if config.Root == "" {
	...
	if err != nil {
		panic(fmt.Sprintf("pisces: %v", err))
	}
			...
			if err != nil {
				if os.IsNotExist(err) {
					if err = next(c); err != nil {
						if he, ok := err.(*pisces.HTTPError); ok {
...
func listDir(t *template.Template, name string, res *pisces.Response) (err error) {
	...
	res.Header().Set(pisces.HeaderContentType, pisces.MIMETextHTMLCharsetUTF8)
	data := struct {
		Name  string
		Files []interface{}
	}{
