...
func TestStatic(t *testing.T) {
	...
	// Directory
	h := StaticWithConfig(config)(pisces.NotFoundHandler)

	assert := assert.New(t)

	if assert.NoError(h(c)) {
		assert.Contains(rec.Body.String(), "pisces")
	}
	...
	if assert.NoError(h(c)) {
		...
		assert.Equal(rec.Header().Get(pisces.HeaderContentLength), "219885")
	}
	...
	h = static(pisces.NotFoundHandler)
	if assert.NoError(h(c)) {
		assert.Equal(http.StatusOK, rec.Code)
		assert.Contains(rec.Body.String(), "pisces")
	}
