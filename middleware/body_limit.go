...
// G, T or P.
func BodyLimit(limit string) pisces.MiddlewareFunc {
...
// See: `BodyLimit()`.
func BodyLimitWithConfig(config BodyLimitConfig) pisces.MiddlewareFunc {
	// Defaults
	...
	if err != nil {
		panic(fmt.Errorf("pisces: invalid body-limit=%s", config.Limit))
	}
	config.limit = limit
	...
	return func(next pisces.HandlerFunc) pisces.HandlerFunc {
		return func(c pisces.Context) error {
			...
			if req.ContentLength > config.limit {
				return pisces.ErrStatusRequestEntityTooLarge
			}
...
func (r *limitedReader) Read(b []byte) (n int, err error) {
	...
	r.read += int64(n)
	if r.read > r.limit {
		return n, pisces.ErrStatusRequestEntityTooLarge
	}
	return
}
...
func (r *limitedReader) Reset(reader io.ReadCloser, context pisces.Context) {
	r.reader = reader
	r.context = context
	r.read = 0
}
